...
async def _run(args, cfg):
    ...
    auth_response = await authenticate_to(
        selected_profile, args.proxy, credentials, display_mode, args.ac_version, args.hostscan_data
    )
...
def authenticate_to(host, proxy, credentials, display_mode, version, hostscan_data):
    logger.info("Authenticating to VPN endpoint", name=host.name, address=host.address)
    return Authenticator(host, proxy, credentials, version, hostscan_data).authenticate(display_mode)
